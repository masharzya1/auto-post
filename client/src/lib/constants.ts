export const MODELS = {
  photos: [
    { id: "gpt-image-1", name: "DALL-E 3", tier: "Standard" },
    { id: "midjourney-v6", name: "Midjourney V6", tier: "Pro" },
    { id: "stable-diffusion-xl", name: "SDXL 1.0", tier: "Standard" }
  ],
  caption: [
    { id: "gpt-4o-mini", name: "GPT-4o Mini", tier: "Standard" },
    { id: "gpt-4o", name: "GPT-4o", tier: "Pro" },
    { id: "claude-3-5-sonnet", name: "Claude 3.5 Sonnet", tier: "Pro" }
  ],
  videos: [
    { id: "luma-dream-machine", name: "Luma Dream Machine", tier: "Pro" },
    { id: "runway-gen-3", name: "Runway Gen-3", tier: "Pro" }
  ]
};
